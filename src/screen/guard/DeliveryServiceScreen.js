...
const DeliveryServiceScreen = ({navigation}) => {
    ...
    const [residentAddressError, setResidentAddressError] = useState("")
    const [additionalNotes, setAdditionalNotes] = useState("")
    const [additionalNotesError, setAdditionalNotesError] = useState("")
    const vehicle = [
        { label: 'Delivery Car', value: 'deliverycar' },
        { label: 'Delivery Van', value: 'delvieryvan' },
        { label: 'Delivery Lorry', value: 'deliverylorry' },
    ];
    ...
    const residentaddressValidation = (data) => {
        const notesRegex = /^$|^[A-Z][A-Z0-9 ]+$/;
        if(data.length > 100000 || !notesRegex.test(data)){
            setResidentAddress(data)
            setResidentAddressError( "Invalid Character for Resident Address field")
        }
        else{
            setResidentAddress(data)
            setResidentAddressError("")
        }
    }
    ...
    return (
        ...
        <MyContainer cardcontainer borderRadius5>
            ...
            <RegisterVisitorForm
                ...
                residentAddressValue={residentAddress}
                residentAddressOnChangeText={residentaddressValidation}
                residentAddressError={residentAddressError}
